...
func NewSignInHandler(authenticationRepo authentication.AuthenticationRepository, logger *log.Logger) *SignInHandler {
...
func (handler *SignInHandler) SignInCallback(ctx *gin.Context) {
	...
	authUserId, err := handler.repository.FindUserIdByProvider(provider, gothUser.UserID)
...
func (handler *SignInHandler) registerAuthUser(ctx *gin.Context, gothUser goth.User) (uuid.UUID, error) {
	userId, err := handler.repository.RegisterAuthUser(gothUser)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Internal Server Error"})
		return uuid.Nil, err
	}
